...
--applying the filter on top of the previous query:
SELECT
product_name,
product_size,
CASE
...
WITH 
total_sales_per_market_date AS (
	SELECT
	market_date,
...
VALUES (24, 'Cheesecake - Japanese', '10"', 3, 'unit', CURRENT_TIMESTAMP);
...
latest_qty AS (
	SELECT
	product_id, quantity,
	row_number() OVER (PARTITION BY product_id ORDER BY market_date DESC) AS x
